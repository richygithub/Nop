...
function getTsType(node) {
    ...
    let e = node.type;
    switch (e) {
        case proto_1.Ast.Type.double:
        case proto_1.Ast.Type.float:
        case proto_1.Ast.Type.int16:
        case proto_1.Ast.Type.int32:
        case proto_1.Ast.Type.int64:
            return "number";
        case proto_1.Ast.Type.string:
            return "string";
        case proto_1.Ast.Type.bool:
            return "boolean";
        case proto_1.Ast.Type.Buffer:
            return "Buffer";
        case proto_1.Ast.Type.json:
            return "object";
        case proto_1.Ast.Type.pb:
        case proto_1.Ast.Type.userDef:
            return node.typeName;
        default:
            throw new Error(`unknown type ${e} in typescript.`);
    }
}
function getTypeDefaultVal(e, isArray = false) {
    if (isArray) {
        return "null";
    }
    switch (e) {
...
let typeVarLen = new Map();
let typeInfo = {};
function isVarLen(node) {
    let t = node.type;
    switch (t) {
        case proto_1.Ast.Type.double:
        case proto_1.Ast.Type.float:
        ...
        case proto_1.Ast.Type.userDef:
            if (typeVarLen.has(node.typeName)) {
                ...
                let userType = typeInfo[node.typeName];
                let ret = userType.fieldList.some(ele => {
                    return (isVarLen(ele) || ele.label == proto_1.Ast.Label.Array);
                });
...
function headerLen(type, userType) {
    switch (type) {
        case proto_1.Ast.Type.json:
        case proto_1.Ast.Type.string:
            return 2;
...
function serialize(node, buf, offset, varname) {
    let t = node.type;
    //    let varname=`${own?own+".":""}${node.name}`;
    let lenval, str;
    switch (t) {
...
function deserialize(node, buf, offset, varname, decvar = " ") {
    let t = node.type;
    //    let varname=`${own?own+".":""}${node.name}`;
    let lenval, str;
    switch (t) {
...
function genUserType(node) {
    let varDefine = genParamDec(node.fieldList, ";");
    let ctorDefine = genParamDec(node.fieldList, ",", true);
    let assign = node.fieldList.map(ele => {
        return `this.${ele.name} = ${ele.name};`;
    }).join("");
    let varLen = node.fieldList.some(ele => { return isVarLen(ele) || ele.label == proto_1.Ast.Label.Array; });
    console.log(`userType:${node.name},${varLen}`);
    let lenStr = getLenStr(node.fieldList, "msg.");
    let ss = genSerializeStr(node.fieldList, "buf", "offset", "msg.");
    let ds = genDeserializeStr(node.fieldList, "buf", "offset", "msg.");
    let varname = "msg";
...
function ImpName(node) {
    return `${firstLetterUpper(node.name)}Imp`;
}
function getDepClass(node) {
    let has = {};
    let dep = [...node.input, node.output].filter(ele => {
        if (ele != null && ele.type == proto_1.Ast.Type.userDef && has[ele.typeName] == null) {
...
function getScopeDepClass(node) {
    let dep = [];
    node.rpc.map(ele => {
...
function getLenStr(nodeList, pre = "") {
    let lenStr = nodeList.map(ele => {
        let elename = `${pre}${ele.name}`;
        if (ele.label == proto_1.Ast.Label.Array) {
...
function genSerializeStr(nodeList, buf, offset, pre = "") {
    let ss = nodeList.map(ele => {
...
function genDeserializeStr(nodeList, buf, offset, pre = "", decvar = "") {
    let ds = nodeList.map(ele => {
        let varname = `${pre}${ele.name}`;
        if (ele.label == proto_1.Ast.Label.Array) {
            let lenval = `_${ele.name}_len`;
...
function genParamDec(nodeList, delim, defval = false) {
    if (nodeList == null)
        return "";
    let str = nodeList.map(ele => {
        if (ele.label == proto_1.Ast.Label.Array) {
...
function genInterface(node, serviceId) {
    // let impStr = `import {${dep.join(",")}} from "../type" `;
    let paramDefine = genParamDec(node.input, ",");
    let funcRetStr = (node.output == null ?
        `Promise<[void,Error]>`
        : `Promise<[${getTsType(node.output)},Error]>`);
    let deReq = genDeserializeStr(node.input, "_buf", "_offset", "", "let ");
...
function writeFile(path, contend, debug = false) {
    if (debug) {
...
let debug = true;
function stubFName(service, node) {
    return `${service}-${node.name}-stub`;
}
function genScope(rootdir, service, dirname, node, inter) {
    let str = "";
...
function genFinalStub(root, type) {
...
exports.genFinalStub = genFinalStub;
//gen Server services 
function genServices(node) {
    let imptStr = "";
    let assignStr = "";
...
//# sourceMappingURL=genCodeTs.js.map